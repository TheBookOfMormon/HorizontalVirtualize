...
public sealed class BlazorVirtualize<TItem> : ComponentBase, IBlazorVirtualizeJsCallbacks, IAsyncDisposable
{
    private BlazorVirtualizeJsInterop? _jsInterop;
    ...
    private string _sizeProperty = "height";
    private string _translationAxis = "Y";
    ...
    ///
    /// The default value is "div". If you are placing the <see cref="BlazorVirtualize{TItem}"/> instance inside
    ...
    protected override void OnParametersSet()
    {
        if (ItemSize <= 1)
        {
            throw new InvalidOperationException(
                $"{GetType()} requires a value of at least 1 for parameter '{nameof(ItemSize)}'.");
        }
        ...
        _sizeProperty = Horizontal ? "width" : "height";
        _translationAxis = Horizontal ? "X" : "Y";
    }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _jsInterop = new BlazorVirtualizeJsInterop(this, JSRuntime);
            await _jsInterop.InitializeAsync(_spacerBefore, _spacerAfter, Horizontal);
        }
    }
...
#pragma warning restore ASP0006 // Do not use non-literal sequence numbers
    ...
    private string GetSpacerStyle(int itemsInSpacer, int numItemsGapAbove)
        ...
        : $"{_sizeProperty}: {(itemsInSpacer * _itemSize).ToString(CultureInfo.InvariantCulture)}px; flex-shrink: 0; transform: translate{_translationAxis}({(numItemsGapAbove * _itemSize).ToString(CultureInfo.InvariantCulture)}px);";

    private string GetSpacerStyle(int itemsInSpacer)
        => $"{_sizeProperty}: {(itemsInSpacer * _itemSize).ToString(CultureInfo.InvariantCulture)}px; flex-shrink: 0;";

    void IBlazorVirtualizeJsCallbacks.OnBeforeSpacerVisible(float spacerSize, float spacerSeparation, float containerSize)
    {
    ...
    void IBlazorVirtualizeJsCallbacks.OnAfterSpacerVisible(float spacerSize, float spacerSeparation, float containerSize)
    {
    ...
    private RenderFragment DefaultPlaceholder(PlaceholderContext context) => (builder) =>
    {
        ...
        builder.AddAttribute(1, "style", $"{_sizeProperty}: {_itemSize.ToString(CultureInfo.InvariantCulture)}px; flex-shrink: 0;");
        builder.CloseElement();
    };
